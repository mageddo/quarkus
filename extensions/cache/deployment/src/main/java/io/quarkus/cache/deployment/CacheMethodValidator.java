...
public class CacheMethodValidator {
    ...
    public static void validateAnnotations(AnnotationStore annotationStore, BeanInfo bean, MethodInfo method,
        ...
        if (validateCacheLoadAnnotation(annotationStore, bean, method, throwables)) {
            methodAnnotationsCount++;
        }
        if (validateCacheStoreAnnotation(annotationStore, bean, method, throwables)) {
            methodAnnotationsCount++;
        }
    ...
    private static boolean validateCacheLoadAnnotation(AnnotationStore annotationStore, BeanInfo bean, MethodInfo method,
            List<Throwable> throwables) {
        return validateCacheAnnotation(annotationStore, method, CACHE_LOAD, () -> {
            checkVoidReturnType(CACHE_LOAD, bean, method).ifPresent(throwables::add);
            checkNonBlockingReturnType(CACHE_LOAD, bean, method).ifPresent(throwables::add);
        });
    }

    private static boolean validateCacheStoreAnnotation(AnnotationStore annotationStore, BeanInfo bean, MethodInfo method,
            List<Throwable> throwables) {
        return validateCacheAnnotation(annotationStore, method, CACHE_STORE, () -> {
            checkVoidReturnType(CACHE_STORE, bean, method).ifPresent(throwables::add);
        });
    }
