...
import io.quarkus.cache.runtime.augmented.AugmentedCacheLoadInterceptor;
import io.quarkus.cache.runtime.augmented.AugmentedCacheStoreInterceptor;
...
class CacheProcessor {
    ...
    List<AdditionalBeanBuildItem> additionalBeans() {
        return Arrays.asList(
                ...
                new AdditionalBeanBuildItem(AugmentedCacheLoadInterceptor.class),
                new AdditionalBeanBuildItem(AugmentedCacheStoreInterceptor.class));
    }
        ...
        for (DotName cacheAnnotation : ALL_CACHE_ANNOTATIONS) {
            for (AnnotationInstance annotation : index.getAnnotations(cacheAnnotation)) {
                if (annotation.target().kind() == METHOD) {
                    cacheNames.add(CacheNameResolver.resolve(annotation));
                }
            }
        }
        return cacheNames;
