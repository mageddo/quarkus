...
Now that the cache was loaded during the previous call, try calling the same URL. This time, you should get a super fast answer with
an `executionTimeInMs` value close to 0.

Let's see what happens if we start from one day in the future using the
`http://localhost:8080/weather?city=Raleigh&daysInFuture=1` URL. You should get an answer two seconds later since two of the
requested days were already loaded in the cache.
...
Quarkus offers four method-level annotations and one argument-level annotation that can be used in a CDI managed bean to
...
parameter which can be used to specify the name of the cache. This is especially useful when the same cache must be accessed
from several classes. If the cache name is not specified, the fully qualified name of the class where the annotated method is
declared will be used as the default cache name.
====
...
[source,properties]
----
quarkus.cache.caffeine.namespace."foo".initial-capacity=10 <1>
quarkus.cache.caffeine.namespace."foo".maximum-size=20 <1>
quarkus.cache.caffeine.namespace."foo".expire-after-write <1>
quarkus.cache.caffeine.namespace."bar".maximum-size=1000 <2>
----
...
----
<1> The cache name is implicitly set to `org.acme.CachedService`.
...
----
<1> The cache key is explicitly composed of two elements. The method signature also contains a third argument which is not part
of the key.
