...
Then, call `http://localhost:8080/weather?city=Raleigh` from a browser. After 6 long seconds, the application will answer
something like this:
...
You can try calling the same URL again and again, it will always take 6 seconds to answer.
...
`http://localhost:8080/weather?city=Raleigh&daysInFuture=1` URL. You should get an answer 2 seconds later since two of the
three requested days were already loaded in the cache.

You can also try calling the same URL with a different city and see the cache in action again. The first call will take 6
