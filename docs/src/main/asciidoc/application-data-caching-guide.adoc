...
When a method annotated with `@CacheResult` is invoked, Quarkus will compute a cache key from the method arguments (or generate a key if the method has no arguments)
...
If several threads try to retrieve a cache value from the same missing key concurrently, the method will only be invoked once.
The first calling thread will trigger the method invocation while the subsequent calling threads will wait for the end of the invocation to get the cached result.
