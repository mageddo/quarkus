...
one day at a time and takes forever to answer. Since the weather forecast is updated once every two hours, caching the service
responses would definitely improve your API performances.
...
First, let's create a service that will simulate an extremely slow call to the external meteorological service. Create
...
something like this (the content will vary depending on the day you run the code):
...
Quarkus offers four method-level annotations and on method parameter-level annotations that can be used on a CDI managed bean
